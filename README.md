...
- **Tìm theo số cột**: Gõ `tên_file tim 5~'đồng yên'` để tìm chuỗi trong cột số 5, không phân biệt hoa thường.
...
- **Nạp file `.md`**: Bot lưu file theo tên, ví dụ `mdquytrinh.md` sẽ dùng tên `mdquytrinh`.
- **Cấu trúc**: Dùng `#` cho mục chính, `##` cho mục con.
- **Hiện danh sách quy trình**: Gõ `mdquytrinh hien` để xem các mục `#`.
- **Tìm quy trình**: Gõ `mdquytrinh tim ~'đại hội chi bộ'` để tìm theo tiêu đề/nội dung, không phân biệt hoa thường.
- **Xem quy trình**: Gõ `mdquytrinh xem 1` để xem mục 1, hoặc `mdquytrinh xem 3 1` để xem mục con 1 của mục 3.
- **Gộp quy trình**: Gõ `mdquytrinh them mdquytrinh2.md` để gộp file nguồn vào file tổng, rồi bot tự xóa file nguồn khỏi bộ nhớ.
...
5. Gửi file Markdown quy trình như `mdquytrinh.md` rồi dùng `hien`, `tim`, `xem` theo cấu trúc trên.
