...
- **Nhập dòng mới**: Gõ `tên_file nhap 1=1 2=1 3=15,5 4=Sương nộp` để thêm dòng mới, `id` sẽ tự tăng nếu có cột `id`.
- **Chọn theo số**: Với cột `muc` và `thuchi`, số bên phải dấu `=` là số thứ tự trong danh sách giá trị duy nhất, ví dụ `1=1` nghĩa là chọn mục đầu tiên.
