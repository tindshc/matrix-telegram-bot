...
- **Hướng dẫn nhập**: `jviec cachnhap` và `jphong cachnhap` để xem hướng dẫn trước khi nhập.
- **Giao việc**: `jviec giao 28/4 Báo cáo ctv ds` hoặc `jviec giao am 10/3 Chạp mã nhà thờ lớn gd`.
- **Địa điểm trong `jviec giao`**: Nếu có địa điểm, viết theo mẫu `jviec giao am 10/3 UBND phường - Chạp mã nhà thờ lớn gd`. Nếu không có địa điểm, chèn `-` ngay sau ngày.
...
- **Danh sách tên bộ phận**: `jphong ds hien` để xem danh sách tên của bộ phận, `jphong ds nhap ld ngamy congtin` hoặc `jphong ds nhap ten=ld,ngamy,congtin` để thêm nhiều tên một lượt.
- **Nhập từng bước**: Ở `jphong nhap gui`, bước `phong` chỉ cần gõ `ds`, `gd`...; số bên dưới chỉ là chọn nhanh, không bắt buộc. Ở `jviec nhap gui`, bước `phong` cũng gõ thẳng `ds`, `gd`...
- **Chọn người trong việc**: Khi dùng `jviec nhap gui`, bước `nguoi` sẽ hiện các tên từ `jphong` của phòng đó, và có thể nhập nhiều số như `1,2`.
- **Địa điểm họp**: Trong `jviec nhap gui`, cột `diadiem` nằm trước `nguoi`; nếu không có thì nhập `-` hoặc `/skip`.
